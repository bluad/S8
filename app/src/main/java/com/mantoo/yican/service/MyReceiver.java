...
import com.mantoo.yican.MessagingActivity;
...
public class MyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_RICHPUSH_CALLBACK.equals(intent.getAction())) {
			...
			renderIntent.setAction("com.mantoo.yican.service.CHANGE_STATUS");
			context.sendBroadcast(renderIntent);

			if(bundle.getString(JPushInterface.EXTRA_EXTRA).contains("2"))
            {
                AppCache.putString(AppCacheKey.message,"2");
            }
            else
            {
                AppCache.putString(AppCacheKey.message,"1");
            }
			...
			if(AppCache.getString(AppCacheKey.message).equals("2"))
			{
	...
	public boolean isAppaLive(Context context) {
		...
		String MY_PKG_NAME = "com.mantoo.yican";
