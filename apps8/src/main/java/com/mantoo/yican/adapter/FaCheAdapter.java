...
public class FaCheAdapter extends ArrayAdapter<ExpressStatus> {
    ...
    public int getItemViewType(int position) {
        ...
        if(express.getWay().equals("直送"))
        {
            return 1;
        }
        else if(express.getWay().equals("中转"))
        {
            return 2;
        }
        if(express.getWay().equals("Direct"))
        {
            return 1;
        }
        else if(express.getWay().equals("Transfer"))
        {
            return 2;
        }
        return position;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        TextView totalfache = (TextView) view.findViewById(R.id.totalfache);
        LinearLayout faCheLi = (LinearLayout)view.findViewById(R.id.faCheLi);
        if(express.getTotalAcount() != null && !express.getTotalAcount().equals("") && (Integer.parseInt(express.getTotalAcount()) > 0))
        {
            totalfache.setText(express.getTotalAcount());
            faCheLi.setVisibility(View.VISIBLE);
        }
